...
def test_quality_compounder_range():
    ...
    from src.screener.engine import load_config, load_universe, apply_filters
    conn = sqlite3.connect(DB_PATH)
    config = load_config()
    df = load_universe(conn, "2024-03")
    from src.screener.engine import compute_winsorised_composite
...
def test_quality_compounder_values():
    ...
    result = apply_filters(df, filters, conn, "2024-03")
    conn.close()
    # All should have ROE >= 15
    assert (result["return_on_equity_pct"] >= 15).all()
    # Non-financial should have D/E < 1
...
def test_peer_percentile_count(conn):
    """peer_percentiles table exists and has data."""
    try:
        count = pd.read_sql("SELECT COUNT(*) as cnt FROM peer_percentiles", conn).iloc[0]["cnt"]
        assert count > 0
    except Exception:
        pytest.skip("peer_percentiles table not yet populated")


def test_peer_groups_11_sheets(conn):
...
def test_it_services_roe_ranking(conn):
    """In IT Services, company with highest ROE has highest ROE percentile."""
    try:
        df = pd.read_sql(
        ...
        max_val_idx = df["value"].idxmax()
        max_pct_idx = df["percentile_rank"].idxmax()
        assert max_val_idx == max_pct_idx, "Highest ROE should have highest percentile"
    except Exception:
        pytest.skip("peer_percentiles not available")


def test_percentile_range(conn):
    """All percentile ranks should be between 0 and 1."""
    try:
...
def test_financial_ratios_row_count(conn):
    """financial_ratios has >= 1100 rows."""
...
def test_fk_integrity(conn):
    ...
    assert len(fk) == 0, f"FK violations: {fk}"

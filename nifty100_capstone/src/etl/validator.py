...
class DataQualityValidator:
    ...
    def validate_companies(self, df: pd.DataFrame) -> None:
        ...
        for idx, row in df.iterrows():
            ...
            if pd.isna(comp_id) or comp_id is None:
                self.log_failure(table, "DQ-14", "CRITICAL", comp_id, None, "company_id", comp_id, "Missing Company ID")
            if pd.isna(ticker) or str(ticker).strip() == "":
                self.log_failure(table, "DQ-14", "CRITICAL", comp_id, None, "ticker", ticker, "Missing Ticker Symbol")

            # DQ-13: Ticker format check
            if not pd.isna(ticker):
                ticker_str = str(ticker).strip()
                if not re.match(r"^[A-Z0-9&\-]+$", ticker_str):
                    self.log_failure(table, "DQ-13", "WARNING", comp_id, None, "ticker", ticker, f"Invalid ticker format: '{ticker}'")
        ...
        if "company_id" in df.columns:
            dupes_id = df[df["company_id"].duplicated(keep=False)]
            for _, row in dupes_id.iterrows():
                comp_id = row.get("company_id")
                self.log_failure(table, "DQ-01", "CRITICAL", comp_id, None, "company_id", comp_id, "Duplicate Company ID (Primary Key violation)")

        if "ticker" in df.columns:
            dupes_ticker = df[df["ticker"].duplicated(keep=False)]
            for _, row in dupes_ticker.iterrows():
                comp_id = row.get("company_id")
                ticker = row.get("ticker")
                self.log_failure(table, "DQ-01", "CRITICAL", comp_id, None, "ticker", ticker, f"Duplicate stock ticker: '{ticker}'")
    ...
    def validate_profit_and_loss(self, df: pd.DataFrame, valid_company_ids: set) -> None:
        ...
        for idx, row in df.iterrows():
            ...
            if not pd.isna(sales) and not pd.isna(expenses) and not pd.isna(operating_profit) and sales > 0:
                ...
                if diff > 10.0 and pct_diff > 1.0: # Check with tolerance
            ...
            if not pd.isna(tax_pct):
                if tax_pct < 0 or tax_pct > 100:
                    self.log_failure(table, "DQ-08", "WARNING", comp_id, yr, "tax_pct", tax_pct, f"Effective tax rate out of bounds: {tax_pct}%")
    ...
    def validate_balancesheet(self, df: pd.DataFrame, valid_company_ids: set) -> None:
        ...
        for idx, row in df.iterrows():
            ...
            if not pd.isna(assets) and not pd.isna(liabilities):
                ...
                if diff > 10.0 and pct_diff > 1.0: # Margin threshold > 1%
    ...
    def validate_cashflow(self, df: pd.DataFrame, valid_company_ids: set) -> None:
        ...
        for idx, row in df.iterrows():
            comp_id = row.get("company_id")
            yr = row.get("year")
            op_cash = row.get("operating_cash")
            inv_cash = row.get("investing_cash")
            fin_cash = row.get("financing_cash")
    ...
    def validate_ratios(self, df: pd.DataFrame, valid_company_ids: set) -> None:
        ...
        for idx, row in df.iterrows():
            ...
            if not pd.isna(de) and de > 15.0:
                self.log_failure(table, "DQ-16", "WARNING", comp_id, yr, "debt_equity", de, f"Outlier detected: Debt-to-Equity ratio is exceptionally high: {de}")
